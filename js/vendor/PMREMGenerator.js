...
class PMREMGenerator {
	 ...
	 * (RGBFormat) or HDR (THREE.RGBEFormat). The ideal input image size is 1k (1024 x 512),
	 ...
	 * (RGBFormat) or HDR (THREE.RGBEFormat). The ideal input cube size is 256 x 256,
	 * as this matches best with the 256 x 256 cubemap output.
	 */
	fromCubemap( cubemap ) {
	...
	_allocateTargets( texture ) { // warning: null texture is valid
		...
		const params = {
			...
			encoding: _isLDR( texture ) ? texture.encoding : THREE.RGBEEncoding,
			depthBuffer: false
		};
	...
	_compileMaterial( material ) {
		...
		const tmpMesh = new THREE.Mesh( _lodPlanes[ 0 ], material );
	...
	_textureToCubeUV( texture, cubeUVRenderTarget ) {
		...
		const mesh = new THREE.Mesh( _lodPlanes[ 0 ], material );
	...
	_halfBlur( targetIn, targetOut, lodIn, lodOut, sigmaRadians, direction, poleAxis ) {
		...
		const blurMesh = new THREE.Mesh( _lodPlanes[ lodOut ], blurMaterial );
...
function _isLDR( texture ) {
	...
	if ( texture === undefined || texture.type !== THREE.UnsignedByteType ) return false;

	return texture.encoding === THREE.LinearEncoding || texture.encoding === THREE.sRGBEncoding || texture.encoding === THREE.GammaEncoding;
...
function _createRenderTarget( params ) {
	...
	const cubeUVRenderTarget = new THREE.WebGLRenderTarget( 3 * SIZE_MAX, 3 * SIZE_MAX, params );
